...
module Examples
  module ThTCH2SUWALLFACTORY
    ...
    def to_su_wall(entities, wall)
      ...
      wall_face.pushpull(wall_build_element.height)
      ...
      doors.each{ |door|
        ...
        door_hole_face.pushpull(door_build_element.height)
        # 把墙挖洞
      ...
      windows.each{ |window|
        ...
        window_hole_face.pushpull(window_build_element.height)
        # 把墙挖洞
    ...
    def create_opening_face(group, build_element)
      ...
      origin = ThTCH2SUGeomUtil.to_su_point3d(build_element.origin)
      pts = []
      pts[0] = origin.offset(x_vector,build_element.length / 2).offset(y_vector,build_element.width / 2)
      pts[1] = origin.offset(x_vector,build_element.length / 2).offset(y_vector,-1 * build_element.width / 2)
      pts[2] = origin.offset(x_vector,-1 * build_element.length / 2).offset(y_vector,-1 * build_element.width / 2)
      pts[3] = origin.offset(x_vector,-1 * build_element.length / 2).offset(y_vector,build_element.width / 2)
    ...
    def create_opening_hole_face(group, build_element,wall_width)
      ...
      origin = ThTCH2SUGeomUtil.to_su_point3d(build_element.origin)
      pts = []
      pts[0] = origin.offset(x_vector,build_element.length / 2).offset(y_vector, (wall_width + 5) / 2)
      pts[1] = origin.offset(x_vector,build_element.length / 2).offset(y_vector,-1 * (wall_width + 5) / 2)
      pts[2] = origin.offset(x_vector,-1 * build_element.length / 2).offset(y_vector,-1 * (wall_width + 5) / 2)
      pts[3] = origin.offset(x_vector,-1 * build_element.length / 2).offset(y_vector,(wall_width + 5) / 2)
